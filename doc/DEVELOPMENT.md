...
- [Rules of meta-layers](#rules-of-meta-layers)
...
2. Build Image with `meta-iQ-extra__confidential`.
    ```bash
    DL_DIR=../downloads SSTATE_DIR=../sstate-cache kas-container --ssh-dir ~/.ssh build \
    meta-innodisk-iq/kas/exmp-q911.yml:meta-innodisk-iq/kas/innodisk-distro-confidential.yml
    ```
...
5. Compress following files into `tar.gz`, and generate snapshot of meta-layer with password.
    - `images`
    - `ostree_repo`
    - `sdk`
    - `sbom`
    ```bash
    # suggest using pigz for parallel process
    tar --transform 's|^|<folder>/|' -cf - * | pigz -p "$(nproc)" > <path>/<file>.tar.gz
    ```
6. Generate md5sum for all compressed files.
    ```bash
    md5sum ./*.tar.gz ./*.zip > <file>.txt
    ```
...
# Rules of meta-layers
- Styles are following [oelint-adv](https://marketplace.visualstudio.com/items?itemName=kweihmann.oelint-vscode).
...
# SBOM
- SBOM generation (SPDX + CycloneDX) is opt-in via `INHERIT += "sbom"`, see the main [README](../README.md) for the kas/bitbake commands and full output paths.
- SPDX files land under:
    ```bash
    build-qcom-wayland/tmp-glibc/deploy/spdx/2.2/
    ```
- CycloneDX SBOM (`bom.json`) and VEX (`vex.json`), generated by the third-party [meta-cyclonedx](https://github.com/iris-GmbH/meta-cyclonedx) layer pulled in via kas/sbom.yml, land under:
    ```bash
    build-qcom-wayland/tmp-glibc/deploy/cyclonedx-export/<image-recipe>/
    ```

# Knowing issue
<details>
<summary><b>eMMC boot takes too much time(about 2 min).</b></summary>
...
- Should becareful about some little difference in device-tree, caused issue and solution are describe at [ubuntu.dtsi](../recipes-bsp/q911/files/ubuntu.dtsi).
